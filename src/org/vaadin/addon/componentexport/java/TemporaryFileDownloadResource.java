...
public class TemporaryFileDownloadResource extends StreamResource {
	 ...
	 */
	public TemporaryFileDownloadResource(final Application application,
			final String fileName, final String contentType, final File tempFile)
			throws FileNotFoundException {
	...
	public DownloadStream getStream() {
		final DownloadStream stream = new DownloadStream(getStreamSource()
				.getStream(), contentType, filename);
	...
	private static class FileStreamResource implements
		 ...
		 *             the file not found exception
		 */
		public FileStreamResource(final File fileToDownload)
				throws FileNotFoundException {
